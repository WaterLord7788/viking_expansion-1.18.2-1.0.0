package com.waterlord7788.viking_expansion.entity.projectile;
...
public class SapphireArrow extends AbstractArrow {
    ...
    private static final EntityDataAccessor<Integer> ID_EFFECT_COLOR = SynchedEntityData.defineId(SapphireArrow.class, EntityDataSerializers.INT);
    private static final byte EVENT_POTION_PUFF = 0;
    private Potion potion = Potions.EMPTY;
    private final Set<MobEffectInstance> effects = Sets.newHashSet();
    private boolean fixedColor;

    public SapphireArrow(Level level, double v, double v1, double v2) {
        super(EntityType.ARROW, v, v1, v2, level);
    }

    // Makes sapphire bow shoot the `EntityType.ARROW`
    public SapphireArrow(Level level, LivingEntity livingEntity) {
        super(EntityType.ARROW, livingEntity, level);
    }

    public SapphireArrow(EntityType<? extends SapphireArrow> entityType, Level level) {
        super(entityType, level);

    }

    public void setEffectsFromItem(ItemStack pStack) {
        if (pStack.is(ModItems.SAPPHIRE_ARROW.get())) {
        ...
        } else if (pStack.is(ModItems.SAPPHIRE_ARROW.get())) {
    ...
    protected ItemStack getPickupItem() {
        if (this.effects.isEmpty() && this.potion == Potions.EMPTY) {
            ...
            ItemStack itemstack = new ItemStack(ModItems.SAPPHIRE_ARROW.get());
